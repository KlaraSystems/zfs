...
uint_t metaslab_allow_deganging_pct = 0;
    ...
    uint64_t *found_size)
{
	...
	if (rs == NULL || (zfs_rs_get_end(rs, rt) - zfs_rs_get_start(rs, rt)) <
		...
		zfs_rs_set_end(&rsearch, rt, MIN(max_possible_size, 1ULL << (hbit +
    ...
    int flags, int allocator)
{
	if (!(flags & METASLAB_ASYNC_ALLOC) ||
	    (flags & METASLAB_DONT_THROTTLE) || tag == NULL)
		return;
    ...
    int allocator)
{
	...
	for (int d = 0; d < ndvas; d++) {
		...
		metaslab_group_t *mg = vdev_lookup_top(spa, vdev)->vdev_mg;
		metaslab_group_allocator_t *mga = &mg->mg_allocator[allocator];
		VERIFYF(zfs_refcount_not_held(&mga->mga_alloc_queue_depth,
		    tag), "for tag %px @ mga %px", tag,
		    &mga->mga_alloc_queue_depth);
	}
#endif
}
...
metaslab_group_alloc_normal(metaslab_group_t *mg, zio_alloc_list_t *zal,
    uint64_t asize, uint64_t max_asize, uint64_t txg, boolean_t want_unique,
    ...
    uint64_t *actual_asize)
{
	...
	offset = metaslab_group_alloc_normal(mg, zal, asize, max_asize, txg,
	    want_unique, dva, d, allocator, try_hard, actual_asize);
    ...
    int flags, zio_alloc_list_t *zal, int allocator)
{
	...
	if (max_psize > psize && max_psize >= metaslab_force_ganging &&
	    metaslab_allow_deganging_pct < 100 &&
	    (random_in_range(100) >= MIN(metaslab_allow_deganging_pct, 100))) {
		max_psize = (metaslab_force_ganging - psize) / 2 + psize;
	} else if (max_psize > psize && max_psize >= metaslab_force_ganging &&
	    metaslab_allow_deganging_pct == 100) {
		psize = max_psize;
	}
...
ZFS_MODULE_PARAM(zfs_metaslab, metaslab_, allow_deganging_pct,  UINT, ZMOD_RW,
	"Percentage of range allocs that will be allowed to degang");
