...
void
dbuf_unredirty(dmu_buf_t *db_fake, dmu_tx_t *tx)
{
	...
	zfs_dbgmsg("Decreased dbuf by %llu to %llu", (u_longlong_t)db->db.db_size, (u_longlong_t)dr->dr_accounted);
	dmu_objset_wontuse_space(dn->dn_objset, -1LL * db->db.db_size, tx);
	mutex_exit(&db->db_mtx);
}
