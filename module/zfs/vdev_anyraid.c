    ...
    anyraid_header_t *out_header, int *out_mapping)
{
	...
	for (int i = 0; i < VDEV_ANYRAID_MAP_COPIES; i++) {
		...
		if ((written_txg < spa_current_txg(spa) && !checkpoint_rb) ||
			...
			zfs_dbgmsg("Anyraid header %d on vdev %s: txg %llu out "
			    ...
			    (u_longlong_t)spa_current_txg(spa),
...
static int
anyraid_open_existing(vdev_t *vd, uint64_t child)
{
	vdev_anyraid_t *var = vd->vdev_tsd;
    ...
    uint64_t *logical_ashift, uint64_t *physical_ashift)
{
	 ...
	 * We also can't open the device if we have enough faulted disks that
	 * we can't allocate a full stripe width, even if there is enough
	 * parity to recover all the data already allocated.
	 */
	if (numerrors > var->vd_nparity ||
	    numerrors > vd->vdev_children - (var->vd_nparity + 1)) {
		vd->vdev_stat.vs_aux = VDEV_AUX_NO_REPLICAS;
		return (lasterror);
	}

	if ((spa_load_state(vd->vdev_spa) == SPA_LOAD_IMPORT ||
	    spa_load_state(vd->vdev_spa) == SPA_LOAD_TRYIMPORT ||
	    spa_load_state(vd->vdev_spa) == SPA_LOAD_RECOVER) ||
	    spa_load_state(vd->vdev_spa) == SPA_LOAD_OPEN) {
		for (uint64_t c = 0; c < vd->vdev_children; c++) {
			vdev_t *cvd = vd->vdev_child[c];
			if (cvd->vdev_open_error != 0)
				continue;
			if ((lasterror = anyraid_open_existing(vd, c)) == 0)
				break;
		}
		if (lasterror)
			return (lasterror);
...
vdev_anyraid_state_change(vdev_t *vd, int faulted, int degraded)
{
	...
	if (faulted > var->vd_nparity ||
	    faulted > vd->vdev_children - (var->vd_nparity + 1)) {
    ...
    uint64_t *good_writes, int flags, vdev_config_sync_status_t status)
{
	...
	// Populate the header
	uint8_t disk_id = 0;
	for (; disk_id < anyraidvd->vdev_children; disk_id++) {
		if (anyraidvd->vdev_child[disk_id] == vd)
			break;
	}
