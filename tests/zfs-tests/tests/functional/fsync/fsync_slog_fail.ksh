...
log_assert "verify fsync() behaviour when the slog fails"
...
typeset SLOGDEV="${LOOPDEVS##* }"
typeset RAIDZDEVS="${LOOPDEVS%% $SLOGDEV}"

log_must zpool create $TESTPOOL raidz1 $RAIDZDEVS log $SLOGDEV
...
# inject a device failure into the slog device
log_must zinject -d $SLOGDEV -e io -T write $TESTPOOL
log_must zinject -d $SLOGDEV -e nxio -T flush $TESTPOOL
log_must zinject -d $SLOGDEV -e nxio -T probe $TESTPOOL

# write with a followup fsync
dd if=$DATAFILE of=/$TESTPOOL/data_file bs=128k count=1 conv=fsync &
fsync_pid=$!

# the pool should not suspend; wait a few seconds to be sure that it does not
...
# now find out what happened to dd. It should have returned success, as the
# fsync() would have fallen back to a pool sync
...
log_must test "$fsync_result" = "success"

log_pos zinject -c all
log_pos zpool clear $TESTPOOL

log_pass "fsync() behaves correctly when the slog fails"
